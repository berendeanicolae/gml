...
void  BufToHex(unsigned char *buf,int size,char *strResult)
{
	int				tr,gr;
	unsigned char	v;
	...
	{
		v=buf[tr]/16;
		if (v<10) strResult[gr++]='0'+v; else strResult[gr++]='a'+(v-10);
		v=buf[tr]%16;
		if (v<10) strResult[gr++]='0'+v; else strResult[gr++]='a'+(v-10);
	}
	strResult[gr]=0;
...
void GML::Utils::MD5::Init()
{
	...
	context.stringResult[0]=0;
}
void GML::Utils::MD5::Update(void *Buffer, unsigned int len)
{
    uint32			t;
    ...
    t = context.bits[0];
    if ((context.bits[0] = t + ((uint32) len << 3)) < t)
...
void GML::Utils::MD5::Final()
{
    ...
    *p++ = 0x80;
    ...
    ((uint32 *) context.in)[14] = context.bits[0];
	...
	BufToHex(context.digest,16,context.stringResult);
}
char*GML::Utils::MD5::GetResult()
{
